...
async function main() {
  ...
  if (majorVersion < MINIMUM_MAJOR_VERSION || majorVersion === MINIMUM_MAJOR_VERSION && minorVersion < MINIMUM_MINOR_VERSION) {
    ...
    console.error(
      chalk.gray(
        "Then, activate the installed version in your terminal with " + chalk.bold("`nvm use`.")
      )
    );
    process.exit(1);
  }
  const program = new Command();
  program.name("convex").usage("<command> [options]").addCommand(login, { hidden: true }).addCommand(init, { hidden: true }).addCommand(reinit, { hidden: true }).addCommand(dev).addCommand(deploy).addCommand(deployments, { hidden: true }).addCommand(run).addCommand(logs).addCommand(data, { hidden: true }).addCommand(typecheck, { hidden: true }).addCommand(auth, { hidden: true }).addCommand(convexImport).addCommand(convexExport, { hidden: true }).addCommand(dashboard).addCommand(docs).addCommand(codegen).addCommand(update).addCommand(logout).addCommand(networkTest, { hidden: true }).addCommand(env, { hidden: true }).addHelpCommand("help <command>", "Show help for given <command>").version(version).configureHelp({ visibleOptions: () => [] }).showHelpAfterError();
  try {
